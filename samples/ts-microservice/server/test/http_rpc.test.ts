...
} from "../src/stub/ENetUC_Settings_Manager.js";
import {
	AsnGetSettingsArgument_Converter,
	AsnGetSettingsResult_Converter,
	AsnSetSettingsArgument_Converter,
	AsnSetSettingsResult_Converter,
} from "../src/stub/ENetUC_Settings_Manager_Converter.js";
...
import { asn1Decode, asn1Encode, ROSEBase } from "../src/stub/TSROSEBase.js";
...
describe("Test HTTP Post RPC calls", () => {
	test("asnGetSettings JSON encoded payload", async () => {
		...
		if (encoded) {
			...
			const decoded = asn1Decode<AsnGetSettingsResult>(
				response.data,
				AsnGetSettingsResult_Converter,
				errors,
				getDecodeContext(),
	...
	test("calling not existing method in JSON", async () => {
		...
		try {
			...
			if (axiosError.response) {
				...
				const errors = new ConverterErrors();
				const decoded = asn1Decode<ROSEReject>(
					axiosError.response.data as object,
					ROSEReject_Converter,
					errors,
					getDecodeContext(),
				) as ROSEReject;
	...
	test("calling not existing method in BER", async () => {
		...
		try {
			...
			if (axiosError.response) {
				...
				const errors = new ConverterErrors();
				const decoded = asn1Decode<ROSEReject>(
					axiosError.response.data as Uint8Array,
					ROSEReject_Converter,
					errors,
					getDecodeContext(),
				) as ROSEReject;
	...
	test("asnSetSettings asnGetSettings JSON encoded payload", async () => {
		const setArgument = new AsnSetSettingsArgument({ settings: { bEnabled: true, u8sUsername: "test" } });
		const errors = new ConverterErrors();
		const transportEncodedSetArgument = asn1Encode(
			EASN1TransportEncoding.JSON,
			setArgument,
			AsnSetSettingsArgument_Converter,
			errors,
		...
		if (transportEncodedSetArgument) {
			...
			const decodedSetResponse = asn1Decode<AsnSetSettingsResult>(
				setResponse.data,
				AsnSetSettingsResult_Converter,
				errors,
				getDecodeContext(),
			...
			const transportEncodedGetArgument = asn1Encode(
				EASN1TransportEncoding.JSON,
				getArgument,
				AsnGetSettingsArgument_Converter,
				errors,
			...
			if (transportEncodedGetArgument) {
				...
				const decodedGetResponse = asn1Decode<AsnGetSettingsResult>(
					getResponse.data,
					AsnGetSettingsResult_Converter,
					errors,
					getDecodeContext(),
