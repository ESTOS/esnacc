...
#define STACK_ENTRY __FILE__,__LINE__,__FUNCTION__

//#define FUNC(_funcStr) const char *_func = _funcStr;
#define FUNC(_funcStr)
...
public:
	...
	InvalidTagException(const char* type, const char* elementName, const char* file, long line_number,
		const char* function) throw();
