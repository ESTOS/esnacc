...
//
// This file was generated by estos esnacc (V7.0.18, 15.09.2026)
...
SJson::Value ROSERejectChoice::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	switch (choiceId)
	{
		case invokedIDCid:
			b["invokedID"] = invokedID->JEnc();
			break;
		case invokednullCid:
			b["invokednull"] = invokednull->JEnc();
			break;
		default:
			throw EXCEPT("Choice is empty", ENCODE_ERROR);
	}
...
SJson::Value ROSEResultSeq::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	b["resultValue"] = resultValue.JEnc();
	b["result"] = result.JEnc();
	return b;
}
...
// [PrintSeqDefCodeJsonEnc]
SJson::Value ROSEAuth::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	b["method"] = method.JEnc();
	b["authdata"] = authdata.JEnc();
	return b;
}
...
// [PrintSeqDefCodeJsonEnc]
SJson::Value ROSEError::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	if (NOT_NULL(sessionID))
		b["sessionID"] = sessionID->JEnc();
	b["invokedID"] = invokedID.JEnc();
	b["error_value"] = error_value.JEnc();
	if (NOT_NULL(error))
		b["error"] = error->JEnc();
	return b;
}
...
// [PrintSeqDefCodeJsonEnc]
SJson::Value ROSEAuthRequest::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	b["auth"] = auth.JEnc();
	if (NOT_NULL(context))
		b["context"] = context->JEnc();
	return b;
}
...
// [PrintSeqDefCodeJsonEnc]
SJson::Value ROSEAuthResult::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	b["authList"] = authList.JEnc();
	if (NOT_NULL(context))
		b["context"] = context->JEnc();
	return b;
}
...
int ROSEInvoke::checkConstraints(ConstraintFailList* pConstraintFails) const
{
	...
	if (timeout)
		timeout->checkConstraints(pConstraintFails);
...
void ROSEInvoke::BDecContent(const AsnBuf& _b, AsnTag /*tag0*/, AsnLen elmtLen0, AsnLen& bytesDecoded)
{
	...
	if (tag1 == MAKE_TAG_ID(CNTX, PRIM, 4))
	{
		elmtLen1 = BDecLen(_b, seqBytesDecoded);
		timeout = new AsnInt();
		timeout->BDecContent(_b, tag1, elmtLen1, seqBytesDecoded);
		tag1 = BDecTag(_b, seqBytesDecoded);
	}
...
// [PrintSeqDefCodeJsonEnc]
SJson::Value ROSEInvoke::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	if (NOT_NULL(sessionID))
		b["sessionID"] = sessionID->JEnc();
	b["invokeID"] = invokeID.JEnc();
	if (NOT_NULL(linked_ID))
		b["linked_ID"] = linked_ID->JEnc();
	if (NOT_NULL(operationName))
		b["operationName"] = operationName->JEnc();
	if (NOT_NULL(authentication))
		b["authentication"] = authentication->JEnc();
	if (NOT_NULL(timeout))
		b["timeout"] = timeout->JEnc();
	b["operationID"] = operationID.JEnc();
	if (NOT_NULL(argument))
		b["argument"] = argument->JEnc();
	return b;
}
...
bool ROSEInvoke::JDec(const SJson::Value& b){
	...
	if (b.isMember("timeout"))
	{
		delete timeout;
		timeout = new AsnInt;
		if (!timeout->JDec(b["timeout"]))
			throw InvalidTagException(typeName(), "decode failed: timeout", STACK_ENTRY);
	}
...
// [PrintSeqDefCodeJsonEnc]
SJson::Value ROSEResult::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	if (NOT_NULL(sessionID))
		b["sessionID"] = sessionID->JEnc();
...
// [PrintChoiceDefCodeJsonEnc]
SJson::Value RejectProblem::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	switch (choiceId)
	{
		case generalProblemCid:
			b["generalProblem"] = generalProblem->JEnc();
			break;
		case invokeProblemCid:
			b["invokeProblem"] = invokeProblem->JEnc();
			break;
		case returnResultProblemCid:
			b["returnResultProblem"] = returnResultProblem->JEnc();
			break;
		case returnErrorProblemCid:
			b["returnErrorProblem"] = returnErrorProblem->JEnc();
			break;
		default:
			throw EXCEPT("Choice is empty", ENCODE_ERROR);
	}
...
// [PrintSeqDefCodeJsonEnc]
SJson::Value ROSEReject::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	if (NOT_NULL(sessionID))
		b["sessionID"] = sessionID->JEnc();
	b["invokedID"] = invokedID.JEnc();
	if (NOT_NULL(reject))
		b["reject"] = reject->JEnc();
	if (NOT_NULL(details))
		b["details"] = details->JEnc();
	if (NOT_NULL(authentication))
		b["authentication"] = authentication->JEnc();
	return b;
}
...
// [PrintChoiceDefCodeJsonEnc]
SJson::Value ROSEMessage::JEnc() const
{
	auto b = SJson::Value(SJson::objectValue);
	switch (choiceId)
	{
		case invokeCid:
			b["invoke"] = invoke->JEnc();
			break;
		case resultCid:
			b["result"] = result->JEnc();
			break;
