...
namespace SNACC
{
	...
	GeneralProblem::GeneralProblem(const int i)
		: AsnInt(i)
	{
	}
	...
	InvokeProblem::InvokeProblem(const int i)
		: AsnInt(i)
	{
	}
	...
	ReturnResultProblem::ReturnResultProblem(const int i)
		: AsnInt(i)
	{
	}
	...
	ReturnErrorProblem::ReturnErrorProblem(const int i)
		: AsnInt(i)
	{
	}
	...
	void ROSEReject::BDecContent(const AsnBuf& _b, AsnTag /*tag0*/, AsnLen elmtLen0, AsnLen& bytesDecoded)
	{
		...
		if (tag1 == MAKE_TAG_ID(UNIV, PRIM, INTEGER_TAG_CODE) || tag1 == MAKE_TAG_ID(UNIV, PRIM, NULLTYPE_TAG_CODE))
		{
			elmtLen1 = BDecLen(_b, seqBytesDecoded);
			invokedID.BDecContent(_b, tag1, elmtLen1, seqBytesDecoded);
			if (seqBytesDecoded == elmtLen0)
		...
		if (tag1 == MAKE_TAG_ID(CNTX, PRIM, 0) || tag1 == MAKE_TAG_ID(CNTX, PRIM, 1) || tag1 == MAKE_TAG_ID(CNTX, PRIM, 2) || tag1 == MAKE_TAG_ID(CNTX, PRIM, 3))
		{
			elmtLen1 = BDecLen(_b, seqBytesDecoded);
			reject = new RejectProblem();
