...
void ROSERejectChoice::JEnc(SJson::Value& b) const
{
	...
	{
		case invokedIDCid:
			invokedID->JEnc(b["invokedID"]);
		break;
		case invokednullCid:
			invokednull->JEnc(b["invokednull"]);
		break;
		default:
			throw EXCEPT("Choice is empty", ENCODE_ERROR);
	}
...
{
	b = SJson::Value(SJson::objectValue);
	resultValue.JEnc(b["resultValue"]);
	result.JEnc(b["result"]);
}
...
{
	b = SJson::Value(SJson::objectValue);
	method.JEnc(b["method"]);
	authdata.JEnc(b["authdata"]);
}
...
void ROSEError::JEnc(SJson::Value& b) const
{
	b = SJson::Value(SJson::objectValue);
	if (NOT_NULL(sessionID))
		sessionID->JEnc(b["sessionID"]);
	invokedID.JEnc(b["invokedID"]);
	error_value.JEnc(b["error_value"]);
	if (NOT_NULL(error))
		error->JEnc(b["error"]);
}
...
void ROSEInvoke::JEnc(SJson::Value& b) const
{
	...
	if (NOT_NULL(sessionID))
		sessionID->JEnc(b["sessionID"]);
	invokeID.JEnc(b["invokeID"]);
	if (NOT_NULL(linked_ID))
		linked_ID->JEnc(b["linked_ID"]);
	if (NOT_NULL(operationName))
		operationName->JEnc(b["operationName"]);
	if (NOT_NULL(authentication))
		authentication->JEnc(b["authentication"]);
	operationID.JEnc(b["operationID"]);
	if (NOT_NULL(argument))
		argument->JEnc(b["argument"]);
}
...
void RejectProblem::JEnc(SJson::Value& b) const
{
	...
	{
		case generalProblemCid:
			generalProblem->JEnc(b["generalProblem"]);
		break;
		case invokeProblemCid:
			invokeProblem->JEnc(b["invokeProblem"]);
		break;
		case returnResultProblemCid:
			returnResultProblem->JEnc(b["returnResultProblem"]);
		break;
		case returnErrorProblemCid:
			returnErrorProblem->JEnc(b["returnErrorProblem"]);
		break;
		default:
			throw EXCEPT("Choice is empty", ENCODE_ERROR);
	}
...
void ROSEReject::JEnc(SJson::Value& b) const
{
	...
	invokedID.JEnc(b["invokedID"]);
	if (NOT_NULL(reject))
		reject->JEnc(b["reject"]);
	if (NOT_NULL(details))
		details->JEnc(b["details"]);
	if (NOT_NULL(authentication))
		authentication->JEnc(b["authentication"]);
}
