...
void AsnBits::Set(size_t numBits)
{
	if (numBits > 0)
	{
		...
		if (bits)
			delete[] bits;
		bits = new unsigned char[octetLen + 1];
		memset(bits, 0, octetLen + 1); // init to zeros
	}
	else
	{
		bitLen = 0;
...
void AsnBits::SetSize(size_t newsize)
{
	...
	size_t iSize = (newsize + 7) / 8;
	tmpBits = new unsigned char[iSize];
	memset(tmpBits, 0x00, iSize);
	if (bitLen < newsize)
	{
		if (bits)
			strncpy_s((char*)tmpBits, iSize, (const char*)bits, length());

		delete[] bits;
		size_t iSize2 = (newsize + 7) / 8 + 1;
		bits = new unsigned char[iSize2];
		memset(bits, 0, iSize2);
		strncpy_s((char*)bits, iSize2, (const char*)tmpBits, ((newsize + 7) / 8));
	}
...
AsnLen AsnBits::EncodeGeneral(AsnBufBits& b) const
{
	...
	unsigned long x = 0;
	unsigned long tempLen = (unsigned long)encLen();
...
AsnLen AsnBits::EncodeWithSizeConstraint(AsnBufBits& b) const
{
	...
	{
		tempLength = (long)iSCLowerBound - (long)bitLen;
...
void AsnBits::DecodeWithSizeConstraint(AsnBufBits& b, AsnLen& bitsDecoded)
{
	...
	memset(bits, 0, ((decodeSize + 7) / 8) + 1);
