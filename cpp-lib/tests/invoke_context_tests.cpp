...
const SnaccTelemetryData* FindOutboundWaitTelemetry(const std::vector<std::shared_ptr<const SnaccTelemetryData>>& entries, const SnaccTelemetryData::Reason reason)
{
	const auto it = std::find_if(entries.begin(), entries.end(), [&](const std::shared_ptr<const SnaccTelemetryData>& entry) {
		return entry && entry->m_Direction == SnaccTelemetryData::Direction::OUTBOUND && entry->m_Stage == SnaccTelemetryData::Stage::OUTBOUND_WAIT && entry->m_Reason == reason;
	});
...
bool HasInboundResponseTelemetryForOperation(const std::vector<std::shared_ptr<const SnaccTelemetryData>>& entries, const unsigned int operationId)
{
	return std::any_of(entries.begin(), entries.end(), [&](const std::shared_ptr<const SnaccTelemetryData>& entry) {
		return entry && entry->m_Direction == SnaccTelemetryData::Direction::INBOUND && entry->m_Stage == SnaccTelemetryData::Stage::INBOUND_RESPONSE &&
			entry->m_uiOperationID == operationId;
	});
}
} // namespace
...
protected:
	...
	void AssertInboundResponseSendReusesHandlerInvokeContext(const TransportEncoding encoding)
	{
		...
		EXPECT_TRUE(ServerInboundObservation().SameInstanceHandlerToTransport())
			<< "inbound response send must reuse the same invoke context instance seen in the handler";
	}
	...
	void AssertInboundHandlerRejectReusesInvokeContext(const TransportEncoding encoding)
	{
		...
		EXPECT_GT(ServerInboundObservation().TransportSendCount(), 0u);
		EXPECT_TRUE(ServerInboundObservation().SameInstanceHandlerToTransport())
			<< "inbound reject send must reuse the same invoke context instance seen in the handler";
	}
	...
	void AssertInboundApplicationErrorReusesInvokeContext(const TransportEncoding encoding)
	{
		...
		EXPECT_GT(ServerInboundObservation().TransportSendCount(), 0u);
		EXPECT_TRUE(ServerInboundObservation().SameInstanceHandlerToTransport())
			<< "inbound error response send must reuse the same invoke context instance seen in the handler";
	}
	...
	void AssertInboundResponseSendFailureKeepsInvokeContextOnAttemptedSend(const TransportEncoding encoding)
	{
		...
		EXPECT_GT(ServerInboundObservation().TransportSendCount(), 0u);
		EXPECT_TRUE(ServerInboundObservation().SameInstanceHandlerToTransport())
			<< "failed inbound response send must still pass the handler invoke context to transport";
	}
	...
	void AssertOutboundEncodeFailureStillUsesCallerContext(const TransportEncoding encoding)
	{
		...
		const long roseResult = m_client.SendInvoke(invokeMsg.GetPtr(), &result, &error, "asnSetSettings", 250, pContext);
	...
	void AssertNoTransportReturnsTransportFailedForInvoke(const TransportEncoding encoding)
	{
		...
		const long roseResult = component.Invoke_asnGetSettings(&argument, &result, &error, 250, pContext);
		EXPECT_EQ(ROSE_TE_TRANSPORTFAILED, roseResult);
		EXPECT_FALSE(pContext->WasPreparedForTelemetry());
	...
	void AssertOutboundTransportFailureKeepsCallerContextOnSendAttempt(const TransportEncoding encoding)
	{
		...
		const long roseResult = m_clientSettingsModule.InvokeGetSettingsWithContext(&argument, &result, &error, pContext, 250);
		EXPECT_EQ(ROSE_TE_TRANSPORTFAILED, roseResult);
	...
	void AssertOutboundTimeoutKeepsCallerContextOnRequestSend(const TransportEncoding encoding)
	{
		...
		const long roseResult = m_clientSettingsModule.InvokeGetSettingsWithContext(&argument, &result, &error, pContext, 25);
	...
	void AssertOutboundMalformedResponseKeepsCallerContextThroughDecodeFailure(const TransportEncoding encoding)
	{
		...
		auto responseFuture = std::async(std::launch::async, [&]() {
			return m_client.SendInvoke(invokeMsg.GetPtr(), &result, &error, "asnGetSettings", 250, pContext);
		});
	...
	void AssertOrphanResultDoesNotBreakPendingInvoke(const TransportEncoding encoding)
	{
		...
		auto pendingInvoke = std::async(std::launch::async, [&]() {
			return m_clientSettingsModule.InvokeGetSettingsWithContext(&argument, &result, &error, pContext, 500);
		});
