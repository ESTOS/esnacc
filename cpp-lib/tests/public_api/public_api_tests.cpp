...
namespace snacclib
{
	namespace
	{
		...
		{
			return std::any_of(entries.begin(), entries.end(), [](const CapturedLogEntry& entry) { return !entry.bError; });
		}

		bool HasInboundNonErrorLog(const std::vector<CapturedLogEntry>& entries)
		{
			return std::any_of(entries.begin(), entries.end(), [](const CapturedLogEntry& entry) { return !entry.bOutbound && !entry.bError; });
		}

		bool HasBerLog(const std::vector<CapturedLogEntry>& entries)
		{
			return std::any_of(entries.begin(), entries.end(), [](const CapturedLogEntry& entry) { return !entry.bError && entry.strPayload.find("\"BER\"") != std::string::npos; });
		}
	...
	class snacclib_PublicApiRuntimeTest : public RuntimeTestBase
	{
	protected:
	...
	TEST(snacclib_PublicApiSmokeTest, TransportEncodingCanBeSetAndReadBack)
	{
		SnaccRoseOperationLookup lookup;
	...
	TEST(snacclib_PublicApiSmokeTest, SetSnaccROSETransportRoutesOutboundBytes)
	{
		SnaccRoseOperationLookup lookup;
	...
	TEST_F(snacclib_PublicApiRuntimeTest, OnBinaryDataBlockLoggingFlagControlsRawPayloadLogging)
	{
		AssertOnBinaryDataBlockLoggingFlagControlsRawPayloadLogging();
	}

	TEST_F(snacclib_PublicApiRuntimeTest, OnBinaryDataBlockResultLoggingFlagControlsResponseLogging)
	{
		AssertOnBinaryDataBlockResultLoggingFlagControlsResponseLogging();
	}

	TEST(snacclib_PublicApiSmokeTest, OnBinaryDataBlockResultDecodeErrorsInvokeHookBer)
	{
		AssertOnBinaryDataBlockResultDecodeErrorsInvokeHook(TransportEncoding::BER);
	}

	TEST(snacclib_PublicApiSmokeTest, OnBinaryDataBlockResultDecodeErrorsInvokeHookJson)
	{
		AssertOnBinaryDataBlockResultDecodeErrorsInvokeHook(TransportEncoding::JSON);
	}

	TEST_F(snacclib_PublicApiRuntimeTest, PauseRoseProcessingBlocksNewOutboundInvokesAndEvents)
	{
	...
	TEST_F(snacclib_PublicApiRuntimeTest, PauseRoseProcessingBlocksInboundDispatchUntilResumed)
	{
		InitializeEndpoints(TransportEncoding::JSON);
	...
	TEST(snacclib_PublicApiSmokeTest, ConfigureFileLoggingWritesAndCanBeDisabled)
	{
		SnaccRoseOperationLookup lookup;
	...
	TEST(snacclib_PublicApiSmokeTest, OperationLookupRegistersAndCleansUp)
	{
		SnaccRoseOperationLookup lookup;
	...
	TEST(snacclib_PublicApiSmokeTest, OperationLookupIgnoresRegistrationAfterSeal)
	{
		SnaccRoseOperationLookup lookup;
	...
	TEST(snacclib_PublicApiSmokeTest, TelemetryDebugTextCoversGroupedRejectReasons)
	{
	...
	TEST(snacclib_PublicApiSmokeTest, TelemetryFinalizeCapturesStoredMetadata)
	{
