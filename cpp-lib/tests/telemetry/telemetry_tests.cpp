...
namespace snacclib
{
	namespace
	{
		bool HasTelemetry(const std::vector<std::shared_ptr<const SnaccTelemetryData>>& entries, const SnaccTelemetryData::Direction direction, const SnaccTelemetryData::Outcome outcome, const SnaccTelemetryData::Stage stage, const SnaccTelemetryData::Reason reason, const std::optional<long> roseResult = std::nullopt)
		{
		...
		const SnaccTelemetryData* FindTelemetry(const std::vector<std::shared_ptr<const SnaccTelemetryData>>& entries, const SnaccTelemetryData::Direction direction, const SnaccTelemetryData::Stage stage, const SnaccTelemetryData::Reason reason)
		{
			const auto it = std::find_if(entries.begin(), entries.end(), [&](const std::shared_ptr<const SnaccTelemetryData>& entry) { return entry && entry->m_Direction == direction && entry->m_Stage == stage && entry->m_Reason == reason; });
		...
		void ExpectOutboundWaitTelemetry(const SnaccTelemetryData* pTelemetry, const SnaccTelemetryData::Outcome outcome, const SnaccTelemetryData::Reason reason, const std::optional<long> roseResult, const bool bExpectRequestData, const bool bExpectResponseData)
		{
	...
	class snacclib_TelemetryRuntimeTest : public RuntimeTestBase
	{
	protected:
		...
		void AssertHappyPathTelemetry(const TransportEncoding encoding)
		{
			...
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::RESULT, SnaccTelemetryData::Stage::OUTBOUND_WAIT, SnaccTelemetryData::Reason::REMOTE_RESULT, ROSE_NOERROR));
			ExpectOutboundWaitTelemetry(FindOutboundWaitTelemetry(entries, SnaccTelemetryData::Reason::REMOTE_RESULT), SnaccTelemetryData::Outcome::RESULT, SnaccTelemetryData::Reason::REMOTE_RESULT, ROSE_NOERROR, true, true);
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::INBOUND, SnaccTelemetryData::Outcome::RESULT, SnaccTelemetryData::Stage::INBOUND_INVOKE, SnaccTelemetryData::Reason::LOCAL_RESULT, ROSE_NOERROR));
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::INBOUND, SnaccTelemetryData::Outcome::RESULT, SnaccTelemetryData::Stage::INBOUND_RESPONSE, SnaccTelemetryData::Reason::REMOTE_RESULT, ROSE_NOERROR));
		}
		...
		void AssertApplicationErrorTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::ERR, SnaccTelemetryData::Stage::OUTBOUND_WAIT, SnaccTelemetryData::Reason::REMOTE_ERROR, ROSE_ERROR_VALUE));
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::INBOUND, SnaccTelemetryData::Outcome::ERR, SnaccTelemetryData::Stage::INBOUND_INVOKE, SnaccTelemetryData::Reason::LOCAL_ERROR, ROSE_NOERROR));
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::INBOUND, SnaccTelemetryData::Outcome::ERR, SnaccTelemetryData::Stage::INBOUND_RESPONSE, SnaccTelemetryData::Reason::REMOTE_ERROR, ROSE_ERROR_VALUE));
		}
		...
		void AssertRejectTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::REJECT, SnaccTelemetryData::Stage::OUTBOUND_WAIT, SnaccTelemetryData::Reason::REMOTE_REJECT, ROSE_REJECT_FUNCTIONMISSING));
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::INBOUND, SnaccTelemetryData::Outcome::REJECT, SnaccTelemetryData::Stage::INBOUND_RESPONSE, SnaccTelemetryData::Reason::REMOTE_REJECT, ROSE_REJECT_FUNCTIONMISSING));
		}
		...
		void AssertDecodeFailureTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(std::any_of(entries.begin(), entries.end(), [](const std::shared_ptr<const SnaccTelemetryData>& entry) { return entry && entry->m_Direction == SnaccTelemetryData::Direction::INBOUND && entry->m_Stage == SnaccTelemetryData::Stage::INBOUND_DECODE && entry->m_Reason == SnaccTelemetryData::Reason::DECODE_FAILED; }));
		}
		...
		void AssertTimeoutTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::UNHANDLED, SnaccTelemetryData::Stage::OUTBOUND_WAIT, SnaccTelemetryData::Reason::TIMEOUT, ROSE_TE_TIMEOUT));
		}
		...
		void AssertWaitSkippedTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::DISPATCHED, SnaccTelemetryData::Stage::OUTBOUND_WAIT, SnaccTelemetryData::Reason::WAIT_SKIPPED, ROSE_NOERROR));
		}
		...
		void AssertResponsePayloadDecodeFailureTelemetry(const TransportEncoding encoding, const bool bErrorEnvelope)
		{
			...
			auto responseFuture = std::async(std::launch::async, [&]() { return m_client.SendInvoke(invokeMsg.GetPtr(), &result, &error, "asnGetSettings", pCtx); });
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::UNHANDLED, SnaccTelemetryData::Stage::OUTBOUND_WAIT, SnaccTelemetryData::Reason::DECODE_FAILED, ROSE_RE_DECODE_FAILED));
			EXPECT_FALSE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, bErrorEnvelope ? SnaccTelemetryData::Outcome::ERR : SnaccTelemetryData::Outcome::RESULT, SnaccTelemetryData::Stage::OUTBOUND_WAIT, bErrorEnvelope ? SnaccTelemetryData::Reason::REMOTE_ERROR : SnaccTelemetryData::Reason::REMOTE_RESULT));
		}
		...
		void AssertEventTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::EVENT, SnaccTelemetryData::Stage::OUTBOUND_SEND, SnaccTelemetryData::Reason::LOCAL_EVENT, ROSE_NOERROR));
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::INBOUND, SnaccTelemetryData::Outcome::EVENT, SnaccTelemetryData::Stage::INBOUND_INVOKE, SnaccTelemetryData::Reason::LOCAL_EVENT, ROSE_NOERROR));
		}
		...
		void AssertTransportSendFailureTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::UNHANDLED, SnaccTelemetryData::Stage::OUTBOUND_SEND, SnaccTelemetryData::Reason::SEND_FAILED, ROSE_TE_TRANSPORTFAILED));
		}
		...
		void AssertShutdownPendingInvokeTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			EXPECT_TRUE(HasTelemetry(entries, SnaccTelemetryData::Direction::OUTBOUND, SnaccTelemetryData::Outcome::UNHANDLED, SnaccTelemetryData::Stage::OUTBOUND_WAIT, SnaccTelemetryData::Reason::SHUTDOWN, ROSE_TE_SHUTDOWN));
		...
		void AssertAsyncApplicationErrorTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			ExpectOutboundWaitTelemetry(FindOutboundWaitTelemetry(entries, SnaccTelemetryData::Reason::REMOTE_ERROR), SnaccTelemetryData::Outcome::ERR, SnaccTelemetryData::Reason::REMOTE_ERROR, ROSE_ERROR_VALUE, true, true);
		}
		...
		void AssertAsyncRejectTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			ExpectOutboundWaitTelemetry(FindOutboundWaitTelemetry(entries, SnaccTelemetryData::Reason::REMOTE_REJECT), SnaccTelemetryData::Outcome::REJECT, SnaccTelemetryData::Reason::REMOTE_REJECT, ROSE_REJECT_FUNCTIONMISSING, true, true);
		}
		...
		void AssertAsyncTimeoutTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			ExpectOutboundWaitTelemetry(FindOutboundWaitTelemetry(entries, SnaccTelemetryData::Reason::TIMEOUT), SnaccTelemetryData::Outcome::UNHANDLED, SnaccTelemetryData::Reason::TIMEOUT, ROSE_TE_TIMEOUT, true, false);
		}
		...
		void AssertAsyncShutdownTelemetry(const TransportEncoding encoding)
		{
			...
			const auto entries = TelemetryEntries();
			ExpectOutboundWaitTelemetry(FindOutboundWaitTelemetry(entries, SnaccTelemetryData::Reason::SHUTDOWN), SnaccTelemetryData::Outcome::UNHANDLED, SnaccTelemetryData::Reason::SHUTDOWN, ROSE_TE_SHUTDOWN, true, false);
		}
	...
	TEST_F(snacclib_TelemetryRuntimeTest, HappyPathTelemetryBer)
	{
		AssertHappyPathTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, HappyPathTelemetryJson)
	{
		AssertHappyPathTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ApplicationErrorTelemetryBer)
	{
		AssertApplicationErrorTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ApplicationErrorTelemetryJson)
	{
		AssertApplicationErrorTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, RejectTelemetryBer)
	{
		AssertRejectTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, RejectTelemetryJson)
	{
		AssertRejectTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, DecodeFailureTelemetryBer)
	{
		AssertDecodeFailureTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, DecodeFailureTelemetryJson)
	{
		AssertDecodeFailureTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, TimeoutTelemetryBer)
	{
		AssertTimeoutTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, TimeoutTelemetryJson)
	{
		AssertTimeoutTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, WaitSkippedTelemetryBer)
	{
		AssertWaitSkippedTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, WaitSkippedTelemetryJson)
	{
		AssertWaitSkippedTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ResultPayloadDecodeFailureTelemetryBer)
	{
		AssertResponsePayloadDecodeFailureTelemetry(TransportEncoding::BER, false);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ResultPayloadDecodeFailureTelemetryJson)
	{
		AssertResponsePayloadDecodeFailureTelemetry(TransportEncoding::JSON, false);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ErrorPayloadDecodeFailureTelemetryBer)
	{
		AssertResponsePayloadDecodeFailureTelemetry(TransportEncoding::BER, true);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ErrorPayloadDecodeFailureTelemetryJson)
	{
		AssertResponsePayloadDecodeFailureTelemetry(TransportEncoding::JSON, true);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, EventTelemetryBer)
	{
		AssertEventTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, EventTelemetryJson)
	{
		AssertEventTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, CustomContextIsClonedAndPreparedBer)
	{
		AssertCustomContextIsClonedAndPrepared(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, CustomContextIsClonedAndPreparedJson)
	{
		AssertCustomContextIsClonedAndPrepared(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, TransportSendFailureTelemetryBer)
	{
		AssertTransportSendFailureTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, TransportSendFailureTelemetryJson)
	{
		AssertTransportSendFailureTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ShutdownPendingInvokeTelemetryBer)
	{
		AssertShutdownPendingInvokeTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, ShutdownPendingInvokeTelemetryJson)
	{
		AssertShutdownPendingInvokeTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, SettingsChangedEventTelemetryBer)
	{
		AssertSettingsChangedEventTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, SettingsChangedEventTelemetryJson)
	{
		AssertSettingsChangedEventTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncHappyPathTelemetryBer)
	{
		AssertAsyncHappyPathTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncHappyPathTelemetryJson)
	{
		AssertAsyncHappyPathTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncApplicationErrorTelemetryBer)
	{
		AssertAsyncApplicationErrorTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncRejectTelemetryJson)
	{
		AssertAsyncRejectTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncTimeoutTelemetryBer)
	{
		AssertAsyncTimeoutTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncTimeoutTelemetryJson)
	{
		AssertAsyncTimeoutTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncShutdownTelemetryJson)
	{
		AssertAsyncShutdownTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncTransportSendFailureTelemetryBer)
	{
		AssertAsyncTransportSendFailureTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncTransportSendFailureTelemetryJson)
	{
		AssertAsyncTransportSendFailureTelemetry(TransportEncoding::JSON);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncRequestDataMatchesSyncInvokeBer)
	{
		AssertAsyncRequestDataMatchesSyncInvoke(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncWaitSkippedTelemetryBer)
	{
		AssertAsyncWaitSkippedTelemetry(TransportEncoding::BER);
	}

	TEST_F(snacclib_TelemetryRuntimeTest, AsyncWaitSkippedTelemetryJson)
	{
