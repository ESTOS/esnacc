...
void BDecIA5StringContent(GenBuf* b, AsnTag tagId, AsnLen len, IA5String* result, AsnLen* bytesDecoded, ENV_TYPE env)
{
...
void BDecIA5String(GenBuf* b, IA5String* result, AsnLen* bytesDecoded, ENV_TYPE env)
{
	AsnTag tag;
	AsnLen elmtLen1;

	if (((tag = BDecTag(b, bytesDecoded, env)) != MAKE_TAG_ID(UNIV, PRIM, IA5STRING_TAG_CODE)) && (tag != MAKE_TAG_ID(UNIV, CONS, IA5STRING_TAG_CODE)))
	{
