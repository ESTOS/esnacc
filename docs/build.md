...
TypeScript ROSE runtime specs live under repository-root `typescript/` (parity with `cpp-lib/tests/` scenario folders). Compiler C++ unit tests live under `compiler/tests/cpp/`. All C++ GTest cases (compiler + cpp-lib runtime) link into one executable: `snacc-cpp-tests`. Runtime sources live under `cpp-lib/tests/<scenario>/` (e.g. `call_flow/`, `invoke_context/`).

VS Code CMake Tools shows CTest cases in a tree when `cmake.ctest.testSuiteDelimiter` is set (see `.vscode/settings.json`). GTest suite names in `snacc-cpp-tests` are prefixed (`snacclib_CallFlowRuntimeTest`, `Compiler_CliParameterTest`, …). With `cmake.ctest.testSuiteDelimiter: "[_.]"` and `testSuiteDelimiterMaxOccurrence: 2`, Test Explorer shows **snacclib** / **Compiler** → fixture → case. Use `cmake.ctest.testConfiguration: Release` so MSVC multi-config builds do not show `*_NOT_BUILT` placeholder tests for Debug.

JavaScript dependencies use **pnpm only** (`packageManager` in root `package.json`). `npm install` is blocked via a `preinstall` hook (`scripts/enforce-pnpm.mjs`). Enable pnpm with `corepack enable`, then run `pnpm install` at the repository root (workspace includes `typescript/`). The `samples/ts-microservice` tree has its own pnpm workspace and the same guard in each package (`check-pm.js`). The glue run scripts assemble an ephemeral `typescript/stub/` copy (gluecode plus sample stubs) so `compiler/back-ends/ts-gen/gluecode/` stays product-only. Run `scripts/run_gluecode_tests.bat` (Windows) or `scripts/run_gluecode_tests.sh` (Linux) after `samples/prepare` has installed node-client dependencies.
