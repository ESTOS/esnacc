...
static void PrintDelphiEnumDefCode(FILE* src, ModuleList* mods, Module* m, TypeDef* td, Type* parent, Type* enumerated)
{
	//	NamedType *e;
	//	enum BasicTypeChoiceId tmpTypeId;
	CNamedElmt* n;
...
void PrintDelphiSeqDefCodeJsonEnc(FILE* src, TypeDef* td, Type* seq)
{
	...
	FOR_EACH_LIST_ELMT(e, seq->basicType->a.sequence)
	{
		...
		if ((basictype == BASICTYPE_SEQUENCE) || (basictype == BASICTYPE_SEQUENCEOF))
