// Run: npx tsx compiler/back-ends/ts-gen/tests/TSROSEBase.invokeTimeout.test.ts
...
import * as ENetUC_Common_Converter from "./workdir/ENetUC_Common_Converter.js";
import { ROSEInvoke, ROSEMessage } from "./workdir/SNACCROSE.js";
import { ROSEInvoke_Converter, ROSEMessage_Converter } from "./workdir/SNACCROSE_Converter.js";
...
import {
	CapturingClientTransport,
	noopInvokeHandler,
	TestRose,
} from "./support/rose_test_transport.js";
...
test("RoseInvokeJsonRoundTripPreservesTimeout", () => {
	...
	const encoded = ROSEInvoke_Converter.toJSON(
		new ROSEInvoke({ invokeID: 1, operationID: 42, timeout: 500 }),
		errors,
	);
	assert.ok(encoded);
...
test("InboundInitCopiesWireInvokeTimeout", () => {
	const ctx = ReceiveInvokeContext.create(new ROSEInvoke({
		invokeID: 1,
		operationID: 43210,
		timeout: 750,
	}));
...
test("OutboundWireInvokeTimeoutEncodesOnInvokeJson", async () => {
	...
	await rose.handleInvoke(
		argument,
		new ENetUC_Common.AsnOptionalParamChoice({}),
		4100,
		"asnTestInvoke",
