...
bool PrintTSROSEOperationDefine(FILE* src, ValueDef* v)
{
	/* just do ints */
	...
	if (IsDeprecatedOperation(v->definedName))
		return false;
...
void PrintTSROSEOperationDefines(FILE* src, ValueDefList* defs)
{
	fprintf(src, "\n// [%s]\n", __FUNCTION__);
	...
	FOR_EACH_LIST_ELMT(vd, defs)
	{
		if (bElementAdded)
			fprintf(src, ",\n");
		bElementAdded = PrintTSROSEOperationDefine(src, vd);
	}
	fprintf(src, "\n}\n");
}
...
void PrintTSROSEInterfaceEntry(FILE* src, ModuleList* mods, ValueDef* vd, bool bAsComment, Module* m)
{
	...
	if (GetROSEDetails(vd, &pszArgument, &pszResult, &pszError, NULL, NULL, NULL, true))
	{
...
bool PrintTSROSEHandlerInterfaceEntry(FILE* src, ModuleList* mods, Module* m, ValueDef* vd, bool bAsComment, bool bInvokes, bool bAddNewLine)
{
	...
	if (bAsComment)
	{
		...
		if (GetOperationComment_UTF8(pszFunction, &operationComment))
		{
			fprintf(src, "/**\n");
			...
			GetSequenceComment_UTF8(pszArgument, &argumentComment);
...
void PrintTSROSEInterface(FILE* src, ModuleList* mods, Module* m)
{
	...
	FOR_EACH_LIST_ELMT(vd, m->valueDefs) {
		if (vd->value->type->basicType->choiceId == BASICTYPE_MACROTYPE) {
			if (IsDeprecatedOperation(vd->definedName))
				continue;
...
void PrintTSROSEOnInvokeswitchCase(FILE* src, ModuleList* mods, Module* m)
{
	...
	FOR_EACH_LIST_ELMT(vd, m->valueDefs)
	{
		if (vd->value->type->basicType->choiceId == BASICTYPE_MACROTYPE) {
			...
			char* pszResult = NULL;
			GetROSEDetails(vd, NULL, &pszResult, NULL, NULL, NULL, NULL, true);
			if (!pszResult)
			{
				addedOne = 1;
				fprintf(src, "\t\t\tcase OperationIDs.OPID_%s:\n", pszFunction);
...
void PrintTSROSECode(FILE* src, ModuleList* mods, Module* m)
{
	...
	PrintTSROSEOperationDefines(src, m->valueDefs);
