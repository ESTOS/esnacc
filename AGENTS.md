...
- **C++ formatting:** after every C++ edit, run `clang-format` on all touched `.h`/`.cpp`/`.c` files before finishing (repo root `.clang-format`, clang-format 18+). See [.cursor/rules/cpp-clang-format.mdc](.cursor/rules/cpp-clang-format.mdc); on Windows use `scripts/resolve-clang-format.ps1`.
